  ...
  chord,
    ...
    if related_to in ("chord", "invchord"):
        ...
        for note_degree in CHORDS[chord_type]:
            ...
            if related_to == "chord":
                result.append(scale_scheme[target_note % len(scale_scheme)] + 12 * math.floor(target_note / len(scale_scheme)))
...
def get_notes(song, notes, octaves, related_to):
    if related_to == "static":
        return notes
    candidate_notes = get_candidate_notes(related_to, song.scale, song.current_chord_type, song.current_chord)
    result = []
    for octave in octaves:
        for required_note in notes:
            ...
            if len(candidate_notes) > rotated:
                result.append(song.root_note + 12 * (octave + octave_shift) + candidate_notes[rotated % len(candidate_notes)] + 12 * math.floor(rotated / len(candidate_notes)))
...
def compute_melotor(melotor, position, base_division, song_data):
    if len(melotor.current_melo) > 0 and position % (melotor.melo_change_div / base_division) != 0:
        return
    chord_notes = [(val - 1 + song_data.current_chord - 1) % 7 for val in CHORDS[song_data.current_chord_type]]
    available_notes = [0, 1, 2, 3, 4, 5, 6]; # TODO: read that from Song Data
    ...
    if melo.length < melotor.melo_length:
        ...
        for idx, note in enumerate(melotor.current_melo):
            change_candidates.extend([idx] * math.max(100 - indexed_probabilities[note], 1))
        how_many_to_change = math.ceil(melotor.melo_change_strength / 100 * melotor.melo_length)
...
def get_available_notes(track, song):
    ...
    shifted = [note + shift for note in candidate_notes] if track.related_to == "chord" else candidate_notes
    result = []
    for note in shifted:
        if note < low_limit:
